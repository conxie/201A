...
```{r}
d = read.spss("C:/Users/Connie/Documents/GitHub/201A/Tamiretal2008ReplicationData.sav", to.data.frame=T)
```
...
```{r}
#problem w/ this is that it changes the dataframe values from 0 to NA. I guess the dumb workaround is that is to post hoc change 0 to "NA" 
filtered_d_CX_version = d %>%
  mutate(DoNotUse = case_when(
    DoNotUse == 1 ~ "1",
    TRUE ~ "0" )) %>%
  filter(DoNotUse != 1)
...
filtered_d = filtered_d %>% 
  select(c("Subject", "Cond"), # Generally important columns for both hypotheses
         ...
         -contains("Intro"), -c("WhichGames", "GameComments"), # removes columns that fit specified criteria
...
my_patterns = c("Friends", "Strangers")

rating_hyp_long_d = rating_hyp_long_d %>%
  mutate(IsRecall = grepl(paste(my_patterns, collapse='|'), Measurement))
...
rating_summary_d = rating_hyp_long_d %>%
  ...
  summarize(MeanRating = mean(Score)) %>%
  ungroup()
...
```{r TamirFig2}
performance_summary_d %>%
  ggplot(aes(x = ConfrontationalGame,
